...
class WebDriverIO extends Helper {
  ...
  */
  ...
  openSelect2(selector) {
    selector = withStrictLocator(selector);
    this.browser.execute(function(selector) {
      jQuery(selector).select2('open');
    }, selector);
  }

  closeSelect2(selector) {
    selector = withStrictLocator(selector);
    this.browser.execute(function(selector) {
  ...
  selectOptionForSelect2(selector, option) {
    selector = withStrictLocator(selector);
    this.browser.execute(function(selector, option) {
      jQuery(selector).val(option).trigger('change');
    }, selector, option);
  }

  seeOptionIsSelectedForSelect2(selector, optionText) {
    selector = withStrictLocator(selector);
    this.browser.execute(function(selector, optionText) {
      return jQuery(selector).val();
    }, selector, optionText)
    .then((data) => {
      console.log(data.value);
      console.log(optionText);
      console.log(data.value === optionText);
      return proceedSeeField.call(this, 'assert', selector, data.value);
    });
  }
